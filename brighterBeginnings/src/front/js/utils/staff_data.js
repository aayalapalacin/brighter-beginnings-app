export const staffData = [
    {
        name: "John Smith",
        role: "Administrator",
        fun_fact: "Loves hiking and has climbed 5 different mountains.",
        credentials: [
            { state_req: "MA state req - Bachelor's degree in Education", staff_cred: "John - Master's degree in Education" },
            { state_req: "MA state req - 3 years of administrative experience", staff_cred: "John - 5 years of administrative experience" }
        ],
        img: "john_smith.jpg"
    },
    {
        name: "Emily Johnson",
        role: "Administrator",
        fun_fact: "Enjoys painting and has her own art studio.",
        credentials: [
            { state_req: "MA state req - Bachelor's degree in Education", staff_cred: "Emily - Master's degree in Education Administration" },
            { state_req: "MA state req - 3 years of administrative experience", staff_cred: "Emily - 4 years of administrative experience" }
        ],
        img: "emily_johnson.jpg"
    },
    {
        name: "Michael Brown",
        role: "Administrator",
        fun_fact: "Plays guitar in a local band.",
        credentials: [
            { state_req: "MA state req - Bachelor's degree in Education", staff_cred: "Michael - Master's degree in Early Childhood Education" },
            { state_req: "MA state req - 3 years of administrative experience", staff_cred: "Michael - 6 years of administrative experience" }
        ],
        img: "michael_brown.jpg"
    },
    {
        name: "Sarah Davis",
        role: "Lead Infant Teacher",
        fun_fact: "Is a certified yoga instructor.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Sarah - 6 credits in child development and psychology" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Sarah - 12 months supervised work experience" }
        ],
        img: "sarah_davis.jpg"
    },
    {
        name: "David Wilson",
        role: "Lead Toddler Teacher",
        fun_fact: "Speaks three languages fluently.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "David - 5 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "David - 18 months supervised work experience" }
        ],
        img: "david_wilson.jpg"
    },
    {
        name: "Laura Martinez",
        role: "Lead Pre School Teacher",
        fun_fact: "Runs marathons regularly.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Laura - 7 credits in child development and psychology" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Laura - 20 months supervised work experience" }
        ],
        img: "laura_martinez.jpg"
    },
    {
        name: "James Taylor",
        role: "Assistant Infant Teacher",
        fun_fact: "Is a black belt in karate.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "James - 3 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "James - 10 months supervised work experience" }
        ],
        img: "james_taylor.jpg"
    },
    {
        name: "Natalie Lee",
        role: "Assistant Toddler Teacher",
        fun_fact: "Enjoys baking and often brings homemade cookies to class.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Natalie - 4 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Natalie - 9 months supervised work experience" }
        ],
        img: "natalie_lee.jpg"
    },
    {
        name: "Alex White",
        role: "Assistant Pre School Teacher",
        fun_fact: "Is an avid bird watcher.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Alex - 5 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Alex - 11 months supervised work experience" }
        ],
        img: "alex_white.jpg"
    },
    {
        name: "Kimberly Green",
        role: "Floater",
        fun_fact: "Plays the violin in a community orchestra.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Kimberly - 6 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Kimberly - 8 months supervised work experience" }
        ],
        img: "kimberly_green.jpg"
    },
    {
        name: "Brian Young",
        role: "Floater",
        fun_fact: "Has traveled to over 15 countries.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Brian - 4 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Brian - 10 months supervised work experience" }
        ],
        img: "brian_young.jpg"
    },
    {
        name: "Olivia King",
        role: "Floater",
        fun_fact: "Enjoys rock climbing on weekends.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Olivia - 5 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Olivia - 9 months supervised work experience" }
        ],
        img: "olivia_king.jpg"
    },
    {
        name: "Daniel Scott",
        role: "Floater",
        fun_fact: "Is a certified scuba diver.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Daniel - 3 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Daniel - 12 months supervised work experience" }
        ],
        img: "daniel_scott.jpg"
    },
    {
        name: "Sophia Hill",
        role: "Floater",
        fun_fact: "Loves photography and has won several awards.",
        credentials: [
            { state_req: "MA state req - 3 credit child dev course", staff_cred: "Sophia - 4 credits in child development" },
            { state_req: "MA state req - 9 months supervised work exp.", staff_cred: "Sophia - 10 months supervised work experience" }
        ],
        img: "sophia_hill.jpg"
    }
]
